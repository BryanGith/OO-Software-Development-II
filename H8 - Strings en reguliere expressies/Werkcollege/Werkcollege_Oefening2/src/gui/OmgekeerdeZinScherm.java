     ...
     public OmgekeerdeZinScherm()
     {
        ...
        btnPalin.setOnAction(new EventHandler<ActionEvent>()
        {
            ...
            public void handle(ActionEvent evt)
            {
                ...
                {
                	mijnString = new MijnString(txfInvoer.getText());
                    txfPalin.setText(String.format("is %seen palindroom", mijnString.isPalindroom()?"" : "g"));
                }
                catch (NullPointerException | IllegalArgumentException | StringIndexOutOfBoundsException e )
                {
                    lblMessage.setVisible(true);
        ...
        txfPalin.setEditable(false);
        
        txfInvoer.setOnAction(e -> reset());
  }
     
    private void reset()
    {
        txfInvoer.setText(""); //clear werkt ook
        txfKeerom.setText("");
        txfPalin.setText("");
    }     
}
